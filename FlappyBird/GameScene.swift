...
//  FlappyBird
//
//  Created by kazuki on 2021/01/31.
//
...
class GameScene: SKScene , SKPhysicsContactDelegate{
    ...
    //itemを取得したときの効果音。
    let itemSound = SKAction.playSoundFileNamed("sound.mp3", waitForCompletion: false)
    ...
    func setupItem() {   // 【課題追加】
        // アイテムの画像を読み込む
        let itemTextureA = SKTexture(imageNamed: "item")
        itemTextureA.filteringMode = .linear
        
        
        
        
        // 移動する距離を計算
        let movingDistanceItem = CGFloat(self.frame.size.height)
        
        // 0〜1までのランダム値を生成
        //let random_x = CGFloat.random(in: 0..<1)

        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x:0 , y: +movingDistanceItem, duration:4)
        
        // 自身を取り除くアクションを作成
        let resetItem = SKAction.moveBy(x:0 , y: -movingDistanceItem, duration:0)
        
        // 上にスクロール->元の位置->上にスクロールと無限に繰り返すアクション
        let itemAnimation = SKAction.repeatForever(SKAction.sequence([moveItem,resetItem]))
        ...
        // スプライトに物理演算を設定する
        item.physicsBody = SKPhysicsBody(rectangleOf: itemTextureA.size())    // ←追加
      ...
      // 衝突した時に回転させない
        item.physicsBody?.allowsRotation = false    // ←追加
    ...
    // SKPhysicsContactDelegateのメソッド。衝突したときに呼ばれる
       func didBegin(_ contact: SKPhysicsContact) {
           ...
           if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            (contact.bodyA.categoryBitMask & itemCategory) == itemCategory || (contact.bodyB.categoryBitMask & itemCategory) == itemCategory {
            // アイテム用の物体と衝突した
            //自身を取り除く処理
